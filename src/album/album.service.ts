...
export class AlbumService {
    ...
    async findOne(id: number): Promise<AlbumDTO> {
        const album = await this.albumRepository.findOne({ relations: ["tracks", "performers", "comments"] }); //#relations: ["tracks", "performers", "comments"]
        if (!album) 
    ...
    async update(id: number, albumDTO: AlbumDTO): Promise<AlbumDTO> {
        ...
        const album = await this.albumRepository.findOne({ relations: ["tracks", "performers", "comments"] });
        if (!album)
            throw new BusinessLogicException("The album with the given id was not found", BusinessError.NOT_FOUND)
        else {
            const { error } = validate(this.schema, albumDTO);
